...
downloadV2Ray(){
    ...
    DOWNLOAD_LINK="https://github.com/Millenniunn/pay-v2ray-sspanel-v3-mod_Uim-plugin/releases/download/${NEW_VER}/v2ray-linux-${VDIS}.zip"
...
getVersion(){
    if [[ -n "$VERSION" ]]; then
        ...
        TAG_URL="https://api.github.com/repos/Millenniunn/pay-v2ray-sspanel-v3-mod_Uim-plugin/releases/latest"
...
installV2Ray(){
    ...
    if [[ ! -f "/etc/v2ray/config.json" ]]; then
        ...
        if [ ! -z "${CFEMAIL}" ]
        then
          sed -i "s|\"kisusan@test.com\"|\"${CFEMAIL}\"|g" "/etc/v2ray/config.json"
